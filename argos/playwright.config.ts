...
// webServer cwd), i.e. `<repo>/argos` → `<repo>/storybook-chromatic`.
// This is the same build Chromatic consumes (`yarn run storybook:chromatic`),
// so Argos captures exactly the `*.visual.ts` surface, not the docs pages.
const STORYBOOK_STATIC = '../storybook-chromatic';
